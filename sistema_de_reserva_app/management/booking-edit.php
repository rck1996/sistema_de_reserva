...
$pdo = app_pdo();
$theme = current_theme();
$evento = fetch_one($pdo->prepare('SELECT * FROM eventos WHERE id_evento = :id'), array(':id' => request_query_int('id_evento')));
...
if ($evento === null) {
    app_redirect('../admin-dashboard.php#reservas', 'Reserva no encontrada');
}
?>
<!doctype html>
<html lang="es">
...
<body class="min-h-screen px-4 py-8 text-slate-900 sm:px-6">
    <main class="mx-auto max-w-4xl rounded-[2rem] border border-white/50 bg-white/85 p-6 shadow-2xl backdrop-blur-xl sm:p-8">
        <a class="text-sm font-medium text-slate-500 hover:text-slate-900" href="../admin-dashboard.php#reservas">← Volver</a>
        <h1 class="mt-4 text-3xl font-semibold">Editar reserva</h1>
        <form action="update.php?id_evento=<?php echo escape_html((string) $evento['id_evento']); ?>&accion=evento" method="post" class="mt-6 grid gap-4 sm:grid-cols-2">
            <?php echo csrf_input(); ?>
            <label class="block text-sm font-medium text-slate-600">
                Cliente
                <select class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3" name="txt_cliente" required>
                    <?php foreach ($clientes as $cliente): ?>
                        <option value="<?php echo escape_html((string) $cliente['id_cliente']); ?>" <?php echo (int) $cliente['id_cliente'] === (int) $evento['id_cliente'] ? 'selected' : ''; ?>>
                            <?php echo escape_html($cliente['nombre_cliente'] . ' ' . $cliente['apellido_cliente']); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </label>
            ...
            <label class="block text-sm font-medium text-slate-600">
                Estado
                <select class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3" name="estado_reserva">
                    <option value="confirmada" <?php echo $evento['estado_reserva'] === 'confirmada' ? 'selected' : ''; ?>>Confirmada</option>
                    <option value="pendiente" <?php echo $evento['estado_reserva'] === 'pendiente' ? 'selected' : ''; ?>>Pendiente</option>
                    <option value="cancelada" <?php echo $evento['estado_reserva'] === 'cancelada' ? 'selected' : ''; ?>>Cancelada</option>
                </select>
            </label>
            <label class="block text-sm font-medium text-slate-600">
                Fecha
